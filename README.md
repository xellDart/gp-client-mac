...
- If the tunnel dies without cleanup (connection drop, crash, forced kill), a root helper at `/usr/local/libexec/gpclient-dns-cleanup` removes the stale `State:/Network/Service/utunN/DNS` entries and the stale static host route to the portal that the vpnc-script left behind, so DNS recovers automatically and reconnection from a different network works. It runs before each connect, on connection-drop detection, after a forced shutdown, and on app launch — and only touches services whose `utun` interface no longer exists
...
$USER ALL=(root) NOPASSWD: /usr/local/libexec/gpclient-dns-cleanup
$USER ALL=(root) NOPASSWD: /usr/bin/true gpclient-sudoers-v3
```

The last line is a version marker: running `sudo -n /usr/bin/true gpclient-sudoers-v3` succeeds only when the current rules are installed, which is how the app detects outdated installs and offers to update them (`sudo -l` cannot distinguish passwordless rules from admin-with-password ones).
